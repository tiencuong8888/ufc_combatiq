...
if st.button("Predict Winner"):
    ...
    if response.status_code == 200:
        ...
        confidence_rate = prediction_data['confidence_rate']
        st.markdown(f"<b><span style='color: white; background-color:black; font-size: 30px'> {fight_outcome}</span></b>", unsafe_allow_html=True)
        st.markdown(f"<b><span style='color: white; background-color:black;font-size: 30px'>Probability: {confidence_rate}</span></b>", unsafe_allow_html=True)
    else:
